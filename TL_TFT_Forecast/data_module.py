...
def build_source_set(source_train, stage):
    target = 'rain' if stage == 1 else 'prcp'
    source_set = TimeSeriesDataSet(
        ...
        time_varying_unknown_reals = ['prcp','rain','temp','rhum','pres','uwnd','vwnd'],
        categorical_encoders = {'id': station_encoder},
        target_normalizer=GroupNormalizer(groups=['id']) if stage == 2 else None,
