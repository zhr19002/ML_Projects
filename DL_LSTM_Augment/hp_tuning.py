...
class LSTMHyperModel(HyperModel):
    def build(self, hp):
        ...
        input_layer = Input(shape=(X_test.shape[1], X_test.shape[2]))
...
for i in range(epochs):
    for n in range(samples):
        ...
        tuner.search(X_train, y_train, epochs=1, batch_size=batch_size, validation_data=(X_val, y_val))