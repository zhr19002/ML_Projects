...
# Data normalization
scaled_stn_wind, scaled_u, scaled_uc, scaled_wave = MinMaxScaler(), MinMaxScaler(), MinMaxScaler(), MinMaxScaler()
...
df_buoy_train.loc[:, 'scaled_u'] = scaled_u.fit_transform(df_buoy_train[['u']])
df_buoy_test.loc[:, 'scaled_u'] = scaled_u.transform(df_buoy_test[['u']])
df_buoy_train.loc[:, scaled_cols[1:]] = scaled_uc.fit_transform(df_buoy_train[cols[1:]])
df_buoy_test.loc[:, scaled_cols[1:]] = scaled_uc.transform(df_buoy_test[cols[1:]])
...
def create_sequences(df_buoy, df_stn, step, output):
    scaled_wind_uc = wind_MLP.predict(df_stn[scaled_cols[1:]], verbose=0)
    wind_T_uc = scaled_uc.inverse_transform(scaled_wind_uc)
    wind_T_u = np.sqrt(wind_T_uc[:, 0]**2 + wind_T_uc[:, 1]**2)
    scaled_wind_u = scaled_u.transform(pd.DataFrame(wind_T_u.reshape(-1, 1), columns=['u']))
    scaled_wind_T = np.hstack((scaled_wind_u, scaled_wind_uc))
    ...
    for i in range(len(df_buoy) - periods + 1):
        ...
        X_seq[step:, 0] = -1
        X_seq[step:, 1:] = scaled_wind_T[(i+step):(i+periods)]
        X.append(X_seq)
...
X_train, y_train = create_sequences(df_buoy_train[features], df_stn_train[scaled_cols], step, output)
